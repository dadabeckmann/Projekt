...
```{r}
file_names <- list.files("data/preise_daten/", full.names = TRUE)

prepare_data <- function(file_path) {
  DT <- fread(file_path)
  DT[, date := stringr::str_extract(date, "\\d{4}-\\d{2}-\\d{2}")]
  DT[,
     .(diesel = round(mean(diesel), 2),
       e5     = round(mean(e5), 2),
       e10    = round(mean(e10), 2)),
     by = date]
}

prices <- file_names %>% purrr::map_dfr(prepare_data)

gathered <- tidyr::gather(prices, "type", "price", -date)

gathered %>% 
  ggplot(aes(as.Date(date), price, colour=type)) +
  geom_line() +
  labs(title = "Price Change", x = "Date", y = "Preise (€)") +
  theme_bw()
```
